...
def is_move_ok(grid, x_from, y_from, x_to, y_to):
    ...
    True
    >>> grid = Grid.build([['r', 's', None], [None, None, None]]) #right ok
    >>> is_move_ok(grid, 1, 0, 2, 0)
    True
    >>> grid = Grid.build([[None, 's' , None], [None, 'r', None]]) #down not ok
    >>> is_move_ok(grid, 1, 0, 1, 1)
    False
    >>> grid = Grid.build([['r', 's', 'r'], [None, None, None]]) #down ok
    >>> is_move_ok(grid, 1, 0, 1, 1)
    True
    >>> grid = Grid.build([['r', 's', None], [None, None, None]]) #left not ok
    >>> is_move_ok(grid, 1, 0, 0, 0)
    False
    """
    if not grid.in_bounds(x_to, y_to):
        return False
    if not grid.get(x_to, y_to) is None:
        return False
    if x_to != x_from and y_to != y_from:
        if not grid.get(x_to, y_from) is None:
            return False
    return True
...
def do_whole_grid(grid, brownian):
    ...
    >>> grid = Grid.build([[None, 's', None], [None, 's', 's'], [None, None, None]])
    >>> do_whole_grid(grid, 0)
    [[None, None, None], [None, 's', None], [None, 's', 's']]
    >>> grid = Grid.build([['s', None, None], ['s', None, 's'], [None, 's', 's']])
    >>> do_whole_grid(grid, 0)
    [[None, None, None], ['s', None, 's'], ['s', 's', 's']]
    """
